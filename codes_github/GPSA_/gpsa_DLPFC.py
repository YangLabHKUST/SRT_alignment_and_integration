...
for sample_choose in range(3):
    ...
    for t in range(N_EPOCHS):
        ...
        if t % PRINT_EVERY == 0 or t == N_EPOCHS - 1:
            ...
            ax2.axis('off')
            plt.draw()
            plt.savefig("../../results/DLPFC/gpsa_Sample{}_DLPFC.png".format(sample_map[sample_choose]))
            plt.pause(1 / 60.0)
            ...
            if t == N_EPOCHS - 1:
                ...
                for vv, curr_X in enumerate(X_list):
                    ...
                    colors = list(adata.obs['layer_guess_reordered'].astype('str').map(layer_to_color_map))
                    colors2 = list(
                        slices[fixed_view_idx].obs['layer_guess_reordered'].astype('str').map(layer_to_color_map))
                ...
                save_path = "../../results/DLPFC/gpsa_template_individual_Sample{}_DLPFC.png".format(sample_map[sample_choose])
                ...
                for vv, curr_X in enumerate(X_list):
                    ...
                    axs[int(vv / 2), int(vv % 2)].invert_yaxis()
                fig.delaxes(axs[1, 1])
                save_path = "../../results/DLPFC/gpsa_sequence_individual_Sample{}_DLPFC.png".format(
                    sample_map[sample_choose])
                plt.savefig(save_path)
                plt.show()

            pd.DataFrame(curr_aligned_coords).to_csv("../../results/DLPFC/gpsa_Sample{}_DLPFC_aligned_coords_st.csv".format(sample_map[sample_choose]))
            pd.DataFrame(view_idx["expression"]).to_csv("../../results/DLPFC/gpsa_Sample{}_DLPFC_view_idx_st.csv".format(sample_map[sample_choose]))
            pd.DataFrame(X).to_csv("../../results/DLPFC/gpsa_Sample{}_DLPFC_X_st.csv".format(sample_map[sample_choose]))
            pd.DataFrame(Y).to_csv("../../results/DLPFC/gpsa_Sample{}_DLPFC_Y_st.csv".format(sample_map[sample_choose]))
            data.write("../../results/DLPFC/gpsa_Sample{}_DLPFC_data_st.h5".format(sample_map[sample_choose]))

            if model.n_latent_gps["expression"] is not None:
                curr_W = model.W_dict["expression"].detach().numpy()
                pd.DataFrame(curr_W).to_csv("../../results/DLPFC/gpsa_Sample{}_DLPFC_W_st.csv".format(sample_map[sample_choose]))
